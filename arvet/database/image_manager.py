...
class DefaultImageManager(ImageManager):
    ...
    def __init__(self, file_path, group_name=''):
        ...
        self._prefix = group_name.rstrip('/')
        self._contexts = 0
        self._storage = None
    ...
    def get_image(self, path: str) -> typing.Union[np.ndarray, None]:
        with self:
            return np.array(self._storage.get(path, default=None))
    ...
    def find_path_for_image(self, data, seed=0):
        """
        ...
        :return:
        """
        return self._prefix + xxhash.xxh64(data, seed=seed).hexdigest()
...
def get() -> ImageManager:
    ...
    if __image_manager is None:
        raise RuntimeWarning("No image manager configured, did you forget to call 'configure'?")
