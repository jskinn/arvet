...
import arvet.simulation.controllers.trajectory_follow_controller as follow


class TestTrajectoryFollowController(arvet.database.tests.test_entity.EntityContract, unittest.TestCase):
    ...
    def make_instance(self, *args, **kwargs):
        kwargs = du.defaults(kwargs, {
            'trajectory': {i + np.random.uniform(-0.2, 0.2): tf.Transform(np.random.uniform(-1000, 1000, 3),
                           ...
                           for i in range(1000)},
            'sequence_type': arvet.core.sequence_type.ImageSequenceType(np.random.randint(0, 2))
        })
