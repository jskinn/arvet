...
from util.dict_utils import defaults, split_period_keys
...
class TestDictUtils(TestCase):
    ...
    def test_split_period_keys(self):
        self.assertEquals(split_period_keys({'a.b': 1}), {'a': {'b': 1}})

    def test_split_period_keys_noop(self):
        self.assertEquals(split_period_keys({'a': 1}), {'a': 1})

    def test_split_period_keys_merges(self):
        self.assertEquals(
            split_period_keys({'a.b': 1, 'a.c': 2, 'a.d': 3}),
            {'a': {'b': 1, 'c': 2, 'd': 3} })
    ...
    def test_split_period_keys_doesnt_override(self):
        self.assertEquals(split_period_keys({'a.b': 1, 'a': 2}), {'a': 2})

    def test_split_period_keys_copies(self):
        base = {'a.a': 1, 'a.b': 2, 'a.c': 3, 'b.a': 4, 'b.b': 5, 'b.c': 6}
        result = split_period_keys(base)
        self.assertEquals(result, {
            'a': {'a': 1, 'b': 2, 'c': 3},
            'b': {'a': 4, 'b': 5, 'c': 6}
        })
